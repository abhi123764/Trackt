...
class _AddTrainerScreenState extends State<AddTrainerScreen> {
  ...
  Future<void> _handleSubmit() async {
    ...
    final salaryVal = double.tryParse(_salaryController.text.trim()) ?? 0.0;

    final provider = context.read<TrainerProvider>();
    final success = await provider.addTrainer(
    ...
    if (success) {
      ScaffoldMessenger.of(context).showSnackBar(
        const SnackBar(
          content: Text('Trainer profile saved successfully!'),
                        ...
                        children: [
                          ...
                          _buildLabeledField(
                            ...
                            child: TextFormField(
                              ...
                              style: _inputStyle,
                              decoration: _inputDecor('e.g 12/07/2025'),
                            ),
                          ),
                          const SizedBox(height: 14),
                          _buildLabeledField(
...
// ── CUSTOM DASHED UPLOAD BOX WIDGET ───────────────────────────────────────────
